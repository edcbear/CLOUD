...
def index(request):
    user = request.user
    file_obj = models.FileInfo.objects.filter(user_id=1, belong_folder='')
    folder_obj = models.FolderInfo.objects.filter(user_id=1, belong_folder='')
    index_list = []
...
def delete_file(request):
    ...
    models.FileInfo.objects.filter(file_path=file_path, user_id=user_id).first().delete()
    try:
...
def rename_file(request):
    ...
    new_file_remarks = request.GET.get('new_file_remarks',None)

    print('new_file_remarks',new_file_remarks)
    ...
    if not request.GET.get('new_file_name', None):
        new_file_name = old_file_name

    new_path = old_path.replace(old_file_name, new_file_name)
    file_obj.file_path = new_path
    file_obj.file_ = new_path

    file_obj.file_remarks = new_file_remarks
...
def upload_file(request):
    ...
    if request.method == "POST":
        ...
        file_remarks = request.POST.get('ps')
        ...
        models.FileInfo.objects.create(user_id=user_obj.id, file_path=file_path,
                                       ...
                                       file_user=user_obj.username, file_remarks=file_remarks,
                                       )
...
def file_type(request):
    ...
    for file in file_obj:
        file_list.append({'file_path': file.file_path,
                          'file_name': file.file_name,
                          'update_time': str(file.update_time),
                          'file_size': file.file_size,
                          'file_type': file.file_type,
                          'file_user': file.file_user,
                          'file_remarks': file.file_remarks,
                          })
